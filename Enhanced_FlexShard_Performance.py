...
# WORKLOADS = [20, 40, 60, 80, 100]  # Full range

WORKLOADS = [2000, 4000, 6000, 8000, 10000]  # Full range
DEFAULT_WORKLOAD = 10000

# VECTOR_FILE_PATH = "/home/mrasheed/mpi_env/VectorDataset/gist-960-euclidean.hdf5"
# FIGURES_DIR = "/home/mrasheed/mpi_env/FlexShard2025/Result_July2025"
...
# CORRECTED FAULT TOLERANCE IMPLEMENTATION
# Addresses all identified issues for 100% authenticity and realistic results

# FINAL REALISTIC FAULT TOLERANCE IMPLEMENTATION
# Fixes all issues to ensure 100% authentic and realistic results
...
FIXED_WORKLOAD = 50000  # Reduced to 50K for faster testing with real data
DEFAULT_NETWORK_SIZE_DIM = 200
...
# CORRECTED: Updated main function integration
# def main_with_fault_tolerance():
#     """CORRECTED: Main function with proper fault tolerance integration"""
#     comm = MPI.COMM_WORLD
#     rank = comm.Get_rank()
#     size = comm.Get_size()
...
#-------------------------------------------------------# #VERSION Final, without baseline comparisions------------------------------------------------------
...
#     # INCLUDE ALL OTHER METHODS FROM VERSION 2...
...
# # ===============================================================
# # VISUALIZATION FUNCTIONS - SAME AS YOUR WORKING CODE
# # ===============================================================
...
#     network_size = results[0][3]
    
#     workload_labels = [f"{int(w/1000)}K" for w in workloads]
    
#     plt.figure(figsize=(12, 7), dpi=300)
#     plt.plot(workloads, throughputs, 'o-', linewidth=3, markersize=12, color='#0072B2')
#     plt.grid(True, linestyle='--', alpha=0.7)
#     plt.xlabel('Workload (Number of Vectors)', fontsize=18, fontweight='bold')
#     plt.ylabel('System Throughput (vectors/second)', fontsize=18, fontweight='bold')
#     plt.title(f'Throughput vs. Workload (Fixed Network: {network_size} nodes)', 
#               fontsize=18, fontweight='bold', pad=20)
    
#     plt.xticks(workloads, workload_labels, fontsize=18)
#     plt.yticks(fontsize=18)
    
#     for i, (w, t) in enumerate(zip(workloads, throughputs)):
#         plt.annotate(f'{t:.0f}', (w, t), xytext=(0, 15), 
...
#     plt.figure(figsize=(12, 7), dpi=300)
#     plt.plot(workloads, latencies, 'o-', linewidth=3, markersize=12, color='#D55E00')
#     plt.grid(True, linestyle='--', alpha=0.7)
#     plt.xlabel('Workload (Number of Vectors)', fontsize=18, fontweight='bold')
#     plt.ylabel('Average Latency (seconds)', fontsize=18, fontweight='bold')
#     plt.title(f'Latency vs. Workload (Fixed Network: {network_size} nodes)', 
#               fontsize=18, fontweight='bold', pad=20)
...
#         plt.annotate(f'{l:.6f}', (w, l), xytext=(0, 15), 
...
#     throughputs = [r[1] for r in results]
#     latencies = [r[2] for r in results]
#     workload = results[0][3]
    
#     plt.figure(figsize=(12, 7), dpi=300)
#     bars = plt.bar(network_sizes, throughputs, color='#0072B2', width=15, alpha=0.8)
#     plt.grid(True, linestyle='--', alpha=0.7, axis='y')
#     plt.xlabel('Network Size (Number of Nodes)', fontsize=18, fontweight='bold')
#     plt.ylabel('System Throughput (vectors/second)', fontsize=18, fontweight='bold')
#     plt.title(f'Throughput vs. Network Size (Fixed Workload: {workload//1000}K vectors)', 
#               fontsize=18, fontweight='bold', pad=20)
...
#     plt.xticks(network_sizes, fontsize=18)
#     plt.yticks(fontsize=18)
...
#     plt.figure(figsize=(12, 7), dpi=300)
#     plt.plot(network_sizes, latencies, 'o-', linewidth=3, markersize=12, color='#D55E00')
#     plt.grid(True, linestyle='--', alpha=0.7)
#     plt.xlabel('Network Size (Number of Nodes)', fontsize=18, fontweight='bold')
#     plt.ylabel('Average Latency (seconds)', fontsize=18, fontweight='bold')
#     plt.title(f'Latency vs. Network Size (Fixed Workload: {workload//1000}K vectors)', 
#               fontsize=18, fontweight='bold', pad=20)
    
#     for i, (n, l) in enumerate(zip(network_sizes, latencies)):
#         plt.annotate(f'{l:.6f}', (n, l), xytext=(0, 15), 
#                      textcoords='offset points', ha='center', fontsize=11, fontweight='bold')
...
# # ===============================================================
# # MAIN FUNCTION - SAME AS YOUR WORKING CODE
# # ===============================================================

# def main():
#     """Your working main function"""
#     comm = MPI.COMM_WORLD
#     rank = comm.Get_rank()
#     size = comm.Get_size()
...
#         if rank == 0:
#             os.makedirs(FIGURES_DIR, exist_ok=True)
#             print(f"FlexShard GIST-960 Experiments")
#             print(f"Dynamic MPI scaling: {size} processes")
#             print(f"Real dataset only - no synthetic data")
#             print("="*60)
        
#         if rank == 0:
#             print("Reading GIST dataset information...")
#             dataset_info = get_dataset_info_safe(VECTOR_FILE_PATH)
#             dataset_shape, dataset_dtype, dataset_key = dataset_info
            
#             print(f"    Dataset: {dataset_shape[0]:,} vectors × {dataset_shape[1]} dimensions")
#             print(f"    Data type: {dataset_dtype}")
#             print(f"    Using key: '{dataset_key}'")
#             total_memory = (dataset_shape[0] * dataset_shape[1] * 4) / (1024**3)
#             print(f"    Total size: {total_memory:.2f} GB")
#             print(f"    Per process: ~{total_memory/size:.2f} GB")
#         else:
#             dataset_info = None
...
#         if rank == 0:
#             print("\n" + "="*80)
#             print("EXPERIMENT 1: Throughput vs Workload (200K-1M vectors)")
#             print("="*80)
        
#         exp1_results = run_experiment_1_optimized(comm, dataset_info, WORKLOADS, DEFAULT_NETWORK_SIZE)
        
#         if rank == 0 and exp1_results:
#             print("\nEXPERIMENT 1 RESULTS:")
#             print(f"{'Workload':<12} {'Throughput':<15} {'Latency':<15} {'Network':<10}")
#             print("-"*55)
#             for workload, throughput, latency, net_size in sorted(exp1_results):
#                 wl_str = f"{workload//1000}K"
#                 print(f"{wl_str:<12} {throughput:.0f} vec/s{'':<5} {latency:.6f}s{'':<3} {net_size} nodes")
            
#             visualize_experiment_1(exp1_results, FIGURES_DIR)
        
#         if rank == 0:
#             print("\n" + "="*80)
#             print("EXPERIMENT 2: Throughput vs Network Size (1M vectors)")
#             print("="*80)
        
#         exp2_results = run_experiment_2_optimized(comm, dataset_info, NETWORK_SIZES, DEFAULT_WORKLOAD)
        
#         if rank == 0 and exp2_results:
#             print("\nEXPERIMENT 2 RESULTS:")
#             print(f"{'Network':<10} {'Throughput':<15} {'Latency':<15} {'Workload':<10}")
#             print("-"*55)
#             for net_size, throughput, latency, workload in sorted(exp2_results):
#                 wl_str = f"{workload//1000}K"
#                 print(f"{net_size} nodes{'':<3} {throughput:.0f} vec/s{'':<5} {latency:.6f}s{'':<3} {wl_str}")
...
#             print("="*60)
#             print("    Real GIST-960 dataset used exclusively")
#             print(f"    Dynamic scaling with {size} MPI processes")
#             print("    Memory-efficient sequential I/O")
#             print("    Authentic performance results")
#             print("="*60)
